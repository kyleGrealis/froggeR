...
Quarto only renders `.qmd` files found in `pages/`. Static assets in `www/` are copied to the output as resources. Rendered HTML goes to `docs/`, keeping build artifacts out of your source directories. Explore the documentation for other [Quarto project](https://quarto.org/docs/projects/quarto-projects.html) types.
...
This is the payoff of a fixed directory layout: the configuration works because the structure is predictable. You do not need to edit `_quarto.yml` to wire up paths. They are already correct. Add more files in the `contents` section as your project grows and follow the example for `Home` for `pages/index.qmd`.
