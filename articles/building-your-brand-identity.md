...
Your documents tell a story through their visual presentation.
Consistent branding makes your work recognizable, professional, and
polished.

The
[`brand_settings()`](https://www.kyleGrealis.com/froggeR/reference/brand_settings.md)
function helps you create and manage a unified visual identity for all
your Quarto projects. Whether you’re a research lab, data science team,
or individual consultant,
[`brand_settings()`](https://www.kyleGrealis.com/froggeR/reference/brand_settings.md)
ensures your documents maintain consistent colors, logos, and
typography.

## What Is Quarto Branding?

Quarto 1.6+ supports branding through `_brand.yml` files, allowing you
to centralize:
...
`froggeR` offers two complementary ways to configure your brand,
balancing ease-of-use with comprehensive customization. This ensures you
can quickly set up core branding elements while retaining full control
over advanced options.

### Core Elements (Interactive)

The most impactful branding elements are configured through an
interactive prompt:
...
For specialized or advanced branding needs, `froggeR` provides access to
the complete `_brand.yml` template (`gists/brand.yml` within the
package). This allows direct editing of all available options,
including:
...
This full template is based on the `gists/brand.yml` file used
internally by
[`write_brand()`](https://www.kyleGrealis.com/froggeR/reference/write_brand.md).
A snippet of the template looks like this:
...
To customize these, select ‘Edit the full template for advanced options’
above.
...
The validator accepts both 6-digit (`#RRGGBB`) and 3-digit (`#RGB`) hex
formats.
...
Choose “Start fresh” to create project-specific branding that doesn’t
affect your global settings.
...
Your editor opens with the complete `_brand.yml` file including your
Core Elements (Interactive) values plus all Advanced Options (Template)
...
Now that you understand branding with
[`brand_settings()`](https://www.kyleGrealis.com/froggeR/reference/brand_settings.md),
explore:
...
- [Set Once, Use
  ...
  Understand global configuration philosophy
...
The
[`brand_settings()`](https://www.kyleGrealis.com/froggeR/reference/brand_settings.md)
function provides a flexible approach to branding:

- **Core Elements (Interactive)**: Quick setup of essential
  elements—name, primary color, logos
- **Advanced Options (Template)**: Advanced customization when you need
...
By configuring branding once with
[`brand_settings()`](https://www.kyleGrealis.com/froggeR/reference/brand_settings.md),
